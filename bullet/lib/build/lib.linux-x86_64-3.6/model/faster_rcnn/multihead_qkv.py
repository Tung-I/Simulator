...
class _qkvRCNN(nn.Module):
    """ faster RCNN """
    def __init__(self, classes, n_way=2, n_shot=5, class_agnostic=True):
        ...
        # self.FFN_layer_1 = nn.Linear(self.d_k * 49, dim_in)
        # init.normal_(self.FFN_layer_1.weight, std=0.01)
        # init.constant_(self.FFN_layer_1.bias, 0)
        # self.FFN_layer_2 = nn.Linear(dim_in, 2)
        # init.normal_(self.FFN_layer_1.weight, std=0.01)
        # init.constant_(self.FFN_layer_1.bias, 0)
        ################
        # few shot settings
    ...
    #     roi_attention_feats = []
    #     current_b = 0
    #     for i, query_feat in enumerate(pooled_feat.chunk(pooled_feat.size(0), dim=0)):
    #         if i == (current_b + 1) * self.num_of_rois:
    #             current_b += 1
    #         n_channel = query_feat.size(1)
    #         current_support_feat = support_feat[current_b].view(-1, n_channel)  # [1, c, 7, 7] -> [49, c]
    #         query_feat = query_feat.view(-1, n_channel)
    #         q_matrix = self.Q_weight(query_feat)  # [49, d_k]
    #         k_matrix = self.K_weight(current_support_feat)
    #         v_matrix = self.V_weight(query_feat)
    #         qk = torch.matmul(q_matrix, k_matrix.transpose(1, 0)) / math.sqrt(self.d_k)  # [49, 49]
    #         qk = F.softmax(qk, 1)
    #         # value = torch.matmul(qk, v_matrix)  # [49, d_k]
    #         value = torch.bmm(qk.unsqueeze(0), v_matrix.unsqueeze(0)).squeeze()
    #         roi_attention_feats += [value.view(-1)]
    #     roi_attention_feats = torch.stack(roi_attention_feats, 0)  # [B*128, 49*d_k]
    #     cls_score = F.relu(self.binary_output_layer(roi_attention_feats))  # [B*128, 2]
    #     cls_prob = F.softmax(cls_score, 1)  # [B*128, 1]
...
class QKVRCNN(_qkvRCNN):
    def __init__(self, classes, num_layers=50, pretrained=False, num_way=2, num_shot=5):
        ...
        _qkvRCNN.__init__(self, classes, n_way=num_way, n_shot=num_shot)
